...
- **声明式显示规则**：首次使用默认提供 Git、GitHub CLI、pnpm、npm、DSH、Python 六条规则；默认规则与自定义规则一样均可编辑、启停或删除，也可展开 JSON 源码批量导入。规则按原始工具名、命令中的可执行文件 basename 与允许的子命令匹配，仅改变显示并保存在当前浏览器。
...
“显示规则”首次使用会加入 Git、GitHub CLI（`gh`）、pnpm、npm、DSH、Python 六条普通规则。默认规则和后来添加的自定义规则使用完全相同的交互：每条规则只显示颜色、徽章、名称和匹配数量，点击摘要后由 Client 原地展开编辑；switch 会立即启停，垃圾桶图标会立即删除。删除后的默认规则不会在刷新后自动补回；即使清空全部，空列表也会持久化。顶部“添加规则”原地打开独立空白编辑态，保存成功后才写入规则。“JSON 源码”保留同一份底层数据，适合批量粘贴、导入或备份。规则按顺序匹配并采用第一条命中项。
